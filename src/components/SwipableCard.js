...
const SwipableCard = ({ article, onPress }) => {
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={onPress} activeOpacity={0.93} style={{ flex: 1 }}>
        {/* Top media area */}
        {images.length > 0 && !imageError ? (
          <View style={styles.imageWrap}>
            ...
            {images.length > 1 && (
              <>
                <TouchableOpacity style={localStyles.leftButton} onPress={() => scrollTo(currentIndex - 1)}>
                  <Ionicons name="chevron-back" size={24} color="#fff" />
                </TouchableOpacity>
                <TouchableOpacity style={localStyles.rightButton} onPress={() => scrollTo(currentIndex + 1)}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.header}>
            <View style={styles.sourceContainer}>
              {article.favicon ? (
                <Image source={{ uri: article.favicon }} style={styles.favicon} />
              ) : null}
              <Text style={styles.sourceText} numberOfLines={1}>
                {article.source}
                {Array.isArray(article.sources) && article.sources.length > 1 && (
                  <Text style={styles.moreSourcesText}>{` +${article.sources.length - 1}`}</Text>
                )}
              </Text>
            </View>
            <View style={styles.metaContainer}>
              <View style={styles.ringWrap}>
                <View style={[styles.ring, { borderColor: getScoreColor(article.score) }]} />
                <Text style={styles.ringText}>{formatScore(article.score)}</Text>
              </View>
            </View>
          </View>
          ...
          {Array.isArray(article.links) && article.links.length > 0 && (
            <View style={styles.linksRow}>
              <Chip icon="link-variant" onPress={() => openLink(article.links[0])} textStyle={styles.linkChipText} style={styles.linkChip}>
                Open source
              </Chip>
              {article.links.length > 1 && (
                <Chip onPress={() => setLinksVisible(true)} textStyle={styles.linkChipText} style={styles.linkChipSecondary}>
          ...
          <View style={styles.footer}>
            <View style={styles.leftFooter}>
              ...
              {article.author ? <Text style={styles.authorText}>by {article.author}</Text> : null}
            </View>
      ...
      {/* Links Bottom Sheet */}
      <Portal>
        <Modal visible={linksVisible} onDismiss={() => setLinksVisible(false)} contentContainerStyle={styles.bottomSheet}>
          <View>
            ...
            {Array.isArray(article.links) && article.links.slice(1).map((u, idx) => {
              ...
              return (
                <TouchableOpacity key={`link-${idx}`} style={styles.linkRow} onPress={() => openLink(u)}>
                  <Image source={{ uri: `https://www.google.com/s2/favicons?domain=${host}&sz=64` }} style={styles.linkFavicon} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.linkHost} numberOfLines={1}>{host}</Text>
